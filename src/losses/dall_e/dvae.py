...
class Dalle_VAE(BasicVAE):
    ...
    def load_model(self):
        if self.dalle_config.model_dir[0] == '/':
            model_dir = self.dalle_config.model_dir
        else:
            model_dir = os.path.join(self.config.user_config.data_dir, self.dalle_config.model_dir)
        encoder = load_model(os.path.join(model_dir, "encoder.pkl"))
        decoder = load_model(os.path.join(model_dir, "decoder.pkl"))
